...
def find_blocks_greedy(data2_filtered, block_ranges, group1, group2,delta_m_mean_threshold=10, min_cpg_count=5, alpha=1.0, beta=0.5):
    """
    Adopt a greedy strategy to optimize and update CpG blocks, without merging blocks
    """

    # Used for storing the new block structure (but without merging)
    new_block_ranges = {}
    ...
    for block, (start_idx, end_idx) in block_ranges.items():
        ...
        # Calculate the maximum and minimum positions within the block
        d_max = block_data["Position"].max() - block_data["Position"].min()
        
        # Use a max heap (sorted by the absolute value of ΔM)
        ...
        while heap:
            ...
            while left_idx >= start_idx or right_idx <= end_idx:
                ...
                # Limit the range to prevent exceeding block_ranges
    ...
    # Update the 'Block' column of data2_filtered
